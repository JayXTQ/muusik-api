...
export default function (app: Hono) {
    app.post("/scrobble", async (c) => {
        ...
        if (!user || !artist || !track || !album || !timestamp) {
            c.status(400);
            return c.json({ success: false, message: "Required information missing" });
        }
        const signature = generateSignature("track.scrobble", user, artist, track, album, timestamp);
        ...
        if (res.status !== 200) {
            return c.json({ success: false, message: "Scrobbling failed", status: res.status });
        }
        c.status(200);
