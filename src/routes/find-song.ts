...
export const find_song = (app: Hono) => {
    app.get("/find-song", async (c) => {
        ...
        try {
            ...
            if (response.status !== 200) {
                ...
                return c.json({ success: false, message: "Error fetching song" });
            }
            ...
            c.status(500);
            return c.json({ success: false, message: "Server error", details: error.message });
        }
