...
export async function fetchSongNamesFromLastFM(query: string): Promise<{ name: string, artist: string }[]> {
    try {
        ...
        if (response.data.results && response.data.results.trackmatches && response.data.results.trackmatches.track) {
            ...
            return tracks.map((track: any) => ({ name: track.name, artist: track.artist }));
        } else {
